...
server.post("/savepoint", (req,res) => {
    ...
    // inserindo os dados no banco:
        ...
        function afterInsertData(err) {
            ...
            //  return res.send("ok") // Agora ele retorna ac confirmação de cadastro
            // no banco só depois de terminar a execução da callback
...
server.get("/search", (req, res) => {
    ...
    // Para procurar e apresentar os dados de forma dinâmica, devemos primeiro
    ...
    // Utilizar o LIKE e o % dentro dos aspas permite a busca por strings que 
    // contenham a string comparada no antes ou depois
    db.all(`SELECT * FROM places WHERE city LIKE '%${search}%'`, function(err, rows) {
        if (err) {
            console.log(err)
            return res.send("Erro de cadastro") // para que a msg de erro apareço,
            // para o usuário também, não só no console
        }
        console.log("Aqui estão seus registros: ")
        console.log(rows)

        const total = rows.length //Conta a quatidade de colunas da tabela,
        // e consequentemente quantas pontos cadastrados 
